...
/* If the password field hasn't been dropped we want to leave it alone to avoid data loss
 *
...
*/
    ...
    if ($oldversion < 2015091000) {
        // revert database structure to match stable version
        $table = new xmldb_table('groupselect_groups_teachers');
        ...
        $table = new xmldb_table('groupselect');
        $field = new xmldb_field('hidefullgroups');
        $dbman->drop_field($table, $field);
        $field = new xmldb_field('deleteemptygroups');
        $dbman->drop_field($table, $field);
        $field = new xmldb_field('studentcancreate');
        $dbman->drop_field($table, $field);
        $field = new xmldb_field('minmembers');
        $dbman->drop_field($table, $field);
        $field = new xmldb_field('assignteachers');
        $dbman->drop_field($table, $field);
        $field = new xmldb_field('studentcansetdesc');
        $dbman->drop_field($table, $field);
        $field = new xmldb_field('showassignedteacher');
        $dbman->drop_field($table, $field);

        $field = new xmldb_field('password', XMLDB_TYPE_CHAR, '255', null, null, null, null, 'targetgrouping');
        ...
        upgrade_mod_savepoint(true, 2015091000, 'groupselect');
    }
